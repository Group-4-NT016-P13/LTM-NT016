...
namespace TCPServer
{
    public partial class TCPServer : Form
    {
        ...
        //  Mã hóa thông điệp nước đi quân cờ
        private void HandleMovePacket(byte[] byteArray)
        {
            try
            {
                // Chuyển đổi mảng byte thành MovePacket
                MovePacket movePacket = MovePacket.FromByteArray(byteArray);

                // Log thông tin di chuyển quân cờ
                LogMessage($"Moving piece from ({movePacket.StartX}, {movePacket.StartY}) to ({movePacket.EndX}, {movePacket.EndY})");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Lỗi khi xử lý MovePacket: {ex.Message}");
            }
        }
       
        // Trạng thái phòng chờ cho 2 client 
        private string JoinRoom(string roomId, Socket clientSocket)
        {
            ...
            if (room.Players.Count < 2)
            {
                ...
                if (room.Players.Count == 2)
                {
                    ...
                    room.SendToClient1("StartGame: Trận đấu bắt đầu! Bạn là Player 1.");
                    room.SendToClient2("StartGame: Trận đấu bắt đầu! Bạn là Player 2.");
                }
